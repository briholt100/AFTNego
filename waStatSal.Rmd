...
##tidytext unnesting of words
```{r unnest for tidytext,echo=T}
title <- tibble(line=1:length(df$JobTitle),text=df$JobTitle)
title %>% unnest_tokens(word,text)
```
...
tidy.titles %>% count(word,sort=T) %>% filter(grepl('chan',word,ignore.case = T))
```
